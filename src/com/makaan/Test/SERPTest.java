...
public class SERPTest {
	...
	public void ValidateSidecards() throws InterruptedException {
		...
		try {
			String res = smw.SideCards("City SERP");
			if (res.contains("Pass")) {
				System.out.println(res);
			} else {
				Assert.assertTrue(false, res);
    ...
    //@Test(enabled = false)
	 @Test(priority = 22)
	public void ValidateSponsoredLink() throws InterruptedException {
		System.out.println("Inside Test Validate SponsoredLink");
		try {
			String res = smw.SponsoredLink();
			if (res.contains("Pass")) {
				System.out.println(res);
			} else {
				Assert.assertTrue(false, res);
			...
			Assert.assertTrue(false, "not able to validate SponsoredLink as element was not found on window" +n);
		} catch (TimeoutException t) {
			Assert.assertTrue(false, "not able to validate SponsoredLink due to time out");
		}
	}
	...
	public void ValidateSortingFilter() throws InterruptedException {
		...
		try {
			String res = smw.SortingFilter("City SERP");
			if (res.contains("Pass")) {
				System.out.println(res);
			} else {
				Assert.assertTrue(false, res);
